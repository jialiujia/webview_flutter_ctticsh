...
- (bool)loadRequest:(NSDictionary<NSString*, id>*)request {
    ...
    NSLog(@"loadRequest:%@", url);
    if([url hasPrefix:@"http"]) {
        [self loadUrl:url];
    } else if([url hasPrefix:@"file:///"]) {
        url = [url stringByReplacingOccurrencesOfString:@"file:///" withString:@""];
        NSURL *findUrl = [[NSURL alloc] initWithString:[NSString stringWithFormat:@"file://%@/Frameworks/App.framework/flutter_assets/%@", [[NSBundle mainBundle] bundlePath], url]];
        NSLog(@"findUrl:%@", findUrl);
        [_webView loadFileURL:findUrl allowingReadAccessToURL:[findUrl URLByDeletingLastPathComponent]];

//        NSString *dirPath = [NSString stringWithFormat:@"file://%@/%@", [[NSBundle mainBundle] bundlePath], @"assets/web"];
//        NSString *filePath = [NSString stringWithFormat:@"%@/%@", dirPath, @"websockets.html"];
//        NSString *path = [[NSBundle mainBundle] pathForResource:filePath ofType:@"html"];
//        NSLog(@"path:%@", filePath);
//        [_webView loadFileURL:[[NSURL alloc] initWithString:filePath] allowingReadAccessToURL:[[NSURL alloc] initWithString:filePath].URLByDeletingLastPathComponent];
